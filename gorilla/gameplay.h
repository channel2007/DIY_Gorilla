...
void gp_update_time(bool setTime){
  ...
  if(hour <10){
    img_NC[hour].x = nc_x[1][hour]; img_NC[2].y = nc_y[1][hour];
    gp_showImg(&canvas_NC, &img_NC[hour]);

  }else{
    img_NC[1].x = nc_x[0][1]; img_NC[1].y = nc_y[0][1];
    gp_showImg(&canvas_NC, &img_NC[1]);

    img_NC[hour%10].x = nc_x[1][hour%10]; img_NC[2].y = nc_y[1][hour%10];
    gp_showImg(&canvas_NC, &img_NC[hour%10]);    
  }
  gp_showImg(&canvas_NC, &img_NC[10]);

  
  img_NC[minute/10].x = nc_x[2][minute/10]; img_NC[minute/10].y = nc_y[2][minute/10];
  gp_showImg(&canvas_NC, &img_NC[minute/10]);
  img_NC[minute%10].x = nc_x[3][minute%10]; img_NC[minute%10].y = nc_y[3][minute%10];
  gp_showImg(&canvas_NC, &img_NC[minute%10]);
  
}

void gp_update_score(int s){
  int sTemp = s;
  int multiple= 0;
  ...
  if(sTemp>=1000){
      multiple = sTemp/1000;
      img_NC[multiple].x = nc_x[0][multiple]+4; img_NC[multiple].y = nc_y[0][multiple];
      gp_showImg(&canvas_NC, &img_NC[multiple]);
      sTemp -= (multiple*1000);
  }
  
  if((sTemp>=100) || (s>100)){
      multiple = sTemp/100;
      img_NC[multiple].x = nc_x[1][multiple]+4; img_NC[multiple].y = nc_y[1][multiple];
      gp_showImg(&canvas_NC, &img_NC[multiple]);
      sTemp -= (multiple*100);
  }
  ...
  if((sTemp>=10) || (s>10)){
      multiple = sTemp/10;
      img_NC[multiple].x = nc_x[2][multiple]; img_NC[multiple].y = nc_y[2][multiple];
      gp_showImg(&canvas_NC, &img_NC[multiple]);
      sTemp -= (multiple*10);
  }
  
  img_NC[sTemp].x = nc_x[3][sTemp]; img_NC[sTemp].y = nc_y[3][sTemp];
  gp_showImg(&canvas_NC, &img_NC[sTemp]);
...
void gp_autoplay(){    
    ...
    if(role_walk_id>=12){
      ...
      if( (role_walk_id==0 && gp_path[1]==1)  ||                        
        ...
        gp_right();            
      }else if (role_walk_id==10 && gp_path[14]==1){                  
...
void gp_set_speed(void){
  ...
  set_move_speed = 1000 - speed_bias;
...
void gp_donkey_update(){
  ...
  if (clockSystem.check(CLOCK_BUCKET_MOVE, set_move_speed) && hook_max>0 && role_flash==-1 && game_mode != MODE_PAUSE){
    ...
    if(donkey_leave == -1){
      ...
      bias = (int)speed_bias / SPEED_BIAS;
      if(bias==0)
          donkey_leave = gl_rand() % (4 - 3 + 1) + 3;
      else if(bias==1)
          donkey_leave = gl_rand() % (3 - 2 + 1) + 2;
      else
          donkey_leave = (gl_rand() % 2) + 1;

    }else{      
      donkey_leave--;                  
      if(score>100 && role_walk_id>=11){
          r = gl_rand() % (5 - 1 + 1) + 1;
          if (r!=1 && r!=3)
              r=2;
      }else{
    ...
    if(role_jump && role_flash==-1 && role_jump_time > 1){
...
void gp_up(void){
  ...
  if (clockSystem.check(CLOCK_WALK, 200)){
...
void gp_game_up(){
  ...
  if (clockSystem.check( CLOCK_ANI_BIRD, 33)){
    ...
    if (ani_BIRD_id==1){
      ani_BIRD_x+=5;
      img_BIRD[ani_BIRD].y = 10;

    
    }else if (ani_BIRD_id==2){
      ani_BIRD_x+=4;    
      img_BIRD[ani_BIRD].y = 105;

    
    }else if (ani_BIRD_id==3){
      ani_BIRD_x+=6;
      img_BIRD[ani_BIRD].y = 208;

    
    }else{
      ani_BIRD_id = gl_rand() % 50;
    }    
    
    if(ani_BIRD_x>320)
      ani_BIRD_x = ani_BIRD_id = 0;    
    img_BIRD[ani_BIRD].x = ani_BIRD_x;
  }
  
  if((ani_BIRD_id>0 && ani_BIRD_id<4) && ani_BIRD_x>0)  
    gp_showImg(&canvas_BIRD, &img_BIRD[ani_BIRD]);
  ...
  if(rid != -1){
    ...
    if(rid<21 && star_flash){
      img_STAR.x = star_x[rid]; img_STAR.y = star_y[rid];
      gp_showImg(&canvas_STAR, &img_STAR);
    }
  }
...
void gp_game_down(){
  ...
  if (clockSystem.check( CLOCK_ANI_FIRE, 33)){
    ani_FIRE++;
    if(ani_FIRE>2)
      ani_FIRE = 0;
  }  
  gp_showImg(&canvas_FIRE, &img_FIRE[ani_FIRE]);
  ...
  if (clockSystem.check( CLOCK_LIFE_ANI, set_move_speed)){
    if (life==2){
      if (life_next<3)
        life_next++;
    }else if (life==1){
...
void gp_power_on_down(){
  ...
  img_NC[1].x = nc_x[0][1]; img_NC[1].y = nc_y[0][1];
  gp_showImg(&canvas_NC, &img_NC[1]);
  img_NC[2].x = nc_x[1][2]; img_NC[2].y = nc_y[1][2];
  gp_showImg(&canvas_NC, &img_NC[2]);

  gp_showImg(&canvas_NC, &img_NC[10]);

  img_NC[0].x = nc_x[2][0]; img_NC[0].y = nc_y[2][0];
  gp_showImg(&canvas_NC, &img_NC[0]);
  img_NC[0].x = nc_x[3][0]; img_NC[0].y = nc_y[3][0];
  gp_showImg(&canvas_NC, &img_NC[0]);

  debugSprite.fillSprite(TFT_GREEN);
  debugSprite.drawString("v1.1",0,0,FONT2);      
  debugSprite.pushToSprite(&backupPage,280,132,TFT_GREEN);
...
void gp_update(){    
  if (clockSystem.check(CLOCK_FLIP, 83)) {    
    ...
    if (clockSystem.check( CLOCK_STAR, 500) && closet_invincible)
      star_flash = !star_flash;
    else if (!closet_invincible)
      star_flash = false;

    if(role_jump){
      role_jump_time++;
      if(role_jump_time>250)
        role_jump_time = 250;
    }
    ...
    if(flip){     
      ...
      //fpsSprite.fillSprite(TFT_GREEN);
      //fpsSprite.drawString("FPS:"+String(fps),0,0,FONT2);
      //fpsSprite.pushToSprite(&backupPage,2,2,TFT_GREEN);
